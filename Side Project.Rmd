...
CalcIndLikelihood <- function(data, individual, initial_probabilities,transition, stayer_mat){
  ...
  if (length(stayer_mat) > 2){
    stayer_vec <- stayer_mat[individual,]
  } else {
    stayer_vec <- stayer_mat 
  }
  
  likelihood <- stayer_vec[1] + stayer_vec[2] + ((1 - stayer_vec[1] - stayer_vec[2]) * forward_sum)
  return(likelihood)
}
```
